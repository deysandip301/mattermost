...
func New(sc ServiceConfig, options ...Option) (*PlatformService, error) {
	...
	// comment out this to bypass the license check
...
// ConfigHash returns a hash representing the server configuration
// Used by the redis cluster implementation
func (ps *PlatformService) ConfigHash() string {
	if hash := ps.clientConfigHash.Load(); hash != nil {
		return hash.(string)
	}
	return ""
}
